document.addEventListener("DOMContentLoaded", () => {
  ...
    // Disable generate button until a name is entered
  ...
        // Start polling for players every 3 seconds
        if (!pollingInterval) {
  ...
          const response = await fetch(
            `https://api.hosting.codeyourfuture.io/games/${gameId}/start`,
            {
              method: "POST",
              headers: {
                "Content-Type": "application/json",
  ...
          startGameNote.textContent = "Game started!";
  ...
          const response = await fetch(
            `https://api.hosting.codeyourfuture.io/games/${gameId}/finish`,
            {
              method: "POST",
              headers: {
                "Content-Type": "application/json",
